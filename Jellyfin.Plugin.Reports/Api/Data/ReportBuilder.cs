...
namespace Jellyfin.Plugin.Reports.Api.Data
{
    ...
    public class ReportBuilder : ReportBuilderBase
    {
        ...
        /// <param name="userDataManager"> Manager for user data. </param>
        public ReportBuilder(ILibraryManager libraryManager, IUserDataManager userDataManager)
            : base(libraryManager)
        {
        ...
        {
            // Store userId for use in GetOption
            if (request is BaseReportRequest baseRequest)
            {
                _currentUserId = baseRequest.UserId;
            }
        ...
        private ReportOptions<BaseItem> GetOption(HeaderMetadata header, string sortField = "")
        {
            ...
            switch (header)
            {
                ...
                case HeaderMetadata.Favorite:
                    option.Column = (i, r) => this.GetFavoriteStatus(i);
                    option.Header.CanGroup = false;
                    option.Header.DisplayType = ReportDisplayType.Export;
                    break;
        ...
        /// <summary> Gets favorite status. </summary>
        /// <param name="item"> The item. </param>
        /// <returns> The favorite status as a string ("x" if favorite, empty otherwise). </returns>
        private string GetFavoriteStatus(BaseItem item)
        {
            if (string.IsNullOrEmpty(_currentUserId) || !Guid.TryParse(_currentUserId, out var userId))
            {
                return string.Empty;
            }

            var userData = _userDataManager.GetUserData(userId, item);
            return this.GetBoolString(userData?.IsFavorite ?? false);
        }
